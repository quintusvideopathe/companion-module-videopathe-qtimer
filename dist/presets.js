...
export function UpdatePresets(self) {
    ...
    const variable = (name) => `$(${moduleId}:${name})`;
    ...
    const displayTimeFeedbacks = [{ feedbackId: 'display_time_dynamic', options: {} }];
    const presets = {};
    ...
    function createActionPreset(id, category, name, text, bgcolor, color, actionId, options = {}, feedbacks = [], size = 'auto') {
        presets[id] = {
            type: 'button',
            category,
            name,
    ...
    function createReadoutPreset(id, name, text, bgcolor, size, feedbacks = []) {
        presets[id] = {
            type: 'button',
    ...
    function clonePresetToCategory(sourceId, cloneId, category, name) {
    ...
    function applyFixedTextSizeToCategories(categories, size) {
    ...
    for (const sound of self.getAvailableAudioSounds()) {
        ...
        createActionPreset(`audio_play_${safeId}`, 'Audio', `Play audio sound: ${sound.label}`, `PLAY\n${label}`, combineRgb(30, 64, 175), combineRgb(255, 255, 255), 'audio_play', { soundId: sound.id, volume: 100 });
    }
    ...
    for (const sound of self.getAvailableAudioSounds().slice(0, 8)) {
        const safeId = sound.id.replace(/[^a-z0-9_-]/gi, '_').toLowerCase();
        clonePresetToCategory(`audio_play_${safeId}`, `ready_audio_play_${safeId}`, readyPageAudioCategory, `Ready page audio: play ${sound.label}`);
        clonePresetToCategory(`audio_play_${safeId}`, `ready_show_audio_play_${safeId}`, readyPageShowCategory, `Ready page show: play ${sound.label}`);
    }
