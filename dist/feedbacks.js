...
import { compareNumbers, compareStrings, getActiveDisplayTimeState, getCurrentPlaylistSession, inferDisplayMode, isTimerFinished, normalizeHexColor, safeNumber, } from './state.js';
...
function accentuateColor([red, green, blue]) {
    return [red, green, blue].map((channel) => Math.min(255, Math.round(channel * 1.2 + 12)));
}
function darkenColor([red, green, blue]) {
    return [red, green, blue].map((channel) => Math.max(0, Math.round(channel * 0.35)));
}
function getContrastingTextColor([red, green, blue]) {
