...
import { clampNumber, countEnabledSessions, formatDuration, getActiveDisplayTimeState, getCurrentPlaylistSession, inferDisplayMode, parseClockParts, safeNumber, splitDurationParts, } from './state.js';
...
export class ModuleInstance extends InstanceBase {
    ...
    disconnectWebSocket(scheduleReconnect) {
    ...
    async refreshAllState() {
        ...
        try {
            ...
            const [statusResponse, playlistResponse, audioResponse] = await Promise.all([
                ...
                fetchJson(`${baseUrl}/api/playlist/state`).catch((error) => {
                    this.log('debug', `Playlist refresh failed: ${this.formatError(error)}`);
                    return undefined;
                }),
                fetchJson(`${baseUrl}/api/audio/settings`).catch((error) => {
    ...
    normalizeAudioSounds(audioResponse) {
        ...
        const dedupedSounds = new Map();
        for (const rawSound of rawSounds) {
            const id = String(rawSound?.id ?? '').trim();
            if (!id || dedupedSounds.has(id)) {
                continue;
            }
            const sourceType = rawSound?.sourceType === 'builtin' || rawSound?.sourceType === 'custom' || rawSound?.sourceType === 'default-file'
                ? rawSound.sourceType
                : undefined;
